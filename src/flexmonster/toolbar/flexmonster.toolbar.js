 ...
 *	Version 2.2
 **/
var FlexmonsterToolbar = function(pivotContainer, pivot, isHTML5, width, labels) {
	var self = this;
	...
	self.updateLabels = function (labels) {
	    ...
	    self.setText(document.querySelector("#fm-tab-open-remote-report > a > span"), Labels.remote_report_mobile);
	...
	self.chartsHandler = function (type) {
	    ...
	    var multiple = options['chartMultipleMeasures'];
	...
	self.checkChartMultipleMeasures = function () {
		...
		var multiple = options['chartMultipleMeasures'];
		var node = document.getElementById("fm-tab-charts-multiple");
		if (node != null) self.disableMultyValues(options['chartType'], multiple, node);
	}
	...
	self.showConnectToRemoteCSVDialog = function() {
		var applyHandler = function() {
		    if (textInput.value.length > 0) {
		        var options = self.pivot.getOptions() || {};
		        var isFlatTable = options["flatView"];
	...
	self.showOptionsDialog = function() {
		...
		if (options.grid.showGrandTotals == "off" || options.grid.showGrandTotals == false) {
			offRowsColsCbx.checked = true;
		} else if (options.grid.showGrandTotals == "on" || options.grid.showGrandTotals == true) {
			onRowsColsCbx.checked = true;
		} else if (options.grid.showGrandTotals == "rows") {
			onRowsCbx.checked = true;
		} else if (options.grid.showGrandTotals == "columns") {
			onColsCbx.checked = true;
		}

		if (options.grid.showTotals == false) {
			noSubTotalsCbx.checked = true;
		} else if (options.grid.showTotals == true) {
	...
	var PopUpWindowMobile = function () {	    
	    ...
	    this.setToolbar = function (buttons) {
	        className = "fm-ui fm-ui-btn fm-ui-mobile fm-header-btn ";
