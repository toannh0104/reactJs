...
(function() {
	...
	function prepareChartInfo(data, type) {
		...
		//number formatting
		var format = data.meta.formats[0];
		if (format != null) {
			var thousandsSeparator = (format["thousandsSeparator"] != undefined && format["thousandsSeparator"] != "");
			if (thousandsSeparator) {
				output.chart.thousandSeparator = format["thousandsSeparator"];
			}

			var decimalSeparator = (format["decimalSeparator"] != undefined && format["decimalSeparator"] != "");
			if (decimalSeparator) {
				output.chart.decimalSeparator = format["decimalSeparator"];
			}

			var decimalPlaces = (format["decimalPlaces"] != undefined && format["decimalPlaces"] != -1);
			if (decimalPlaces) {
				output.chart.decimals = format["decimalPlaces"];
				output.chart.forceDecimals = "1";
			}

			var currencySymbol = format["currencySymbol"] != undefined && format["currencySymbol"] != "";
			if (currencySymbol) {
				if (format["currencySymbolAlign"] == "left") {
					output.chart.numberPrefix = format["currencySymbol"];
				} else if (format["currencySymbolAlign"] == "right") {
					output.chart.numbersuffix = format["currencySymbol"];
				}
			}
		}
		//"snumberprefix": "$" - for the 2nd y axis - TBD
		//"snumbersuffix": "%" - for the 2nd y axis - TBD
